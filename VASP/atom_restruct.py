...
class AtomRestruct:
    ...
    def read_poscar(self, filename):
        ...
        with open(filename, 'r') as f:
            ...
            atom_struct = []
            print(poscar_data)
            poscar_data['atom_order'] = [
                pair[0] for pair in poscar_data['conf'] for i in range(pair[1])
            ]
    ...
    def parse_coords(self, coords, coords_type, lattice_matrix,
        ...
        try:
            fin_coord = np.array([float(i) for i in match])
    ...
    def extract_conf_num(self, header):
        return [(atom, int(num)) for atom, num in zip(
            ...
            re.sub(self.regex, '\b', header[1]).split('\b')) if atom != '']
    ...
    def lattice_positions(self, poscar, infer, axis=-1):
        ...
        if pos == len(positions):
            ...
            lattice.insert(pos, (new_pos, sym))
            lattice[pos +
                    1:] = map(lambda x: ([*x[0][:2], x[0][2] + zshift], x[1]),
                              lattice[pos + 1:])
        self.print_lattice(lattice, index_highlight=pos)
        # update maximum zshift
        poscar['basis'][2][
            2] = poscar['basis'][2][2] + zshift / poscar['scaler']
    ...
    def print_lattice_space(self, lattice, axis=1, index_highlight=-1):
        if type(index_highlight) == list:
            index_highlight = [
                i for i in range(index_highlight[0], index_highlight[1])
            ]
        else:
        ...
        unique_plane.sort()
        spacing = {unique_plane[i]: '  ' * i for i in range(len(unique_plane))}
        c = 0
    ...
    def translate_vectors(self, poscar, shift):
        ...
        lattice[shift[0]:shift[1]] = map(
            lambda v: ([v[0][0] + x, v[0][1] + y, v[0][2] + z], v[1]),
    ...
    def transform_coordinates(self, coord_set, theta, phi):
        return [
            rot_matrix_x(phi) @ rot_matrix_z(theta) @ np.array(coord)
    ...
    def structure_generator(self,
        ...
        meta_lattice = np.max(
            [self.lattice_constants[atom][0] for atom in atoms])
            ...
            for atom, monolayers in zip(atoms, planes):
                ...
                for monolayer in range(monolayers):
                    ...
                    try:
                        ...
                        plane_spacing = current_lattice_constants[2] * np.sqrt(
                            3) / 3
                        # modify plane spacing if we are at the interface (don't take 0, always resolve for monolayers-1)
                        if monolayer == monolayers - 1:
                            foreign_spacing = self.lattice_constants[atoms[
                                atm_cnt - 1]][2] * np.sqrt(3) / 3
                            # take the mean
                            plane_spacing = (plane_spacing +
                                             foreign_spacing) / 2
                            print(
                                f"Calculated interface spacing: {atoms[atm_cnt-1]}/{atoms[atm_cnt]} Å",
                    ...
                    for position in current_plane:
                        ...
                        result.append(
                            (*(current_position_base[:2] *
                               current_lattice_constants[:2] -
                               np.array([meta_lattice, meta_lattice]) /
                               2).tolist(), zshift))
                    i += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--infer',
                        help='infer x, y coords',
                        action='store_true')
    parser.add_argument('-s',
                        '--shift',
                        help='shift vector in range [start, stop)',
                        nargs=2,
                        type=int)
    parser.add_argument('--offset',
                        help='offset in z dim for structure builder',
                        type=float)
    parser.add_argument('--flat',
                        help='flat display with coordinates',
                        action='store_true')
    ...
    if args.path:
        ...
        ar.construct_direct_path(spec=args.path,
                                 out=args.out,
                                 shift=args.offset)
        quit()
    if args.flat:
