...
class VASPmanager:
    def __init__(self, filename):
        ...
        if self.copy:
            ...
            print("COPYING FILES {} from {} to {}".format(self.files_to_copy, self.src_dir, self.dst_dir))
            self.copy_and_recurse_sbatch(self.src_dir, self.dst_dir, force=True)
        if self.replace:
            self.check_dst_dir()
            for filename, regex, substitution in self.replacements:
                print("CURRENTLY REPLACING {} for {} in files: {}".format(regex, substitution, filename))
